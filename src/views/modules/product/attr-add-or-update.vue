    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" label-width="120px">
      ...
      <el-form-item label="属性类型" prop="attrType">
        <el-select v-model="dataForm.attrType" placeholder="请选择">
          <el-option v-if="type==1" label="规格参数" :value="1"></el-option>
          <el-option v-if="type==0" label="销售属性" :value="0"></el-option>
          <el-option label="既是销售属性又是规格参数" :value="2"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="所属分组" prop="attrGroupId" v-if="type == 1">
        <!-- 查询跟v-model绑定的数值一样key值对象默认第一个键值对 -->
        ...
        <attrgroup-select :attrGroupId.sync="dataForm.attrGroupId"></attrgroup-select>
      </el-form-item>
...
import AttrgroupSelect from "../common/attrgroup-select";
export default {
  components: { CategoryCascader, AttrgroupSelect },
  props: {
    type: {
      type: Number,
      default: 1
  ...
  mounted() {
    this.subscribe = PubSub.subscribe("catPath", (msg, path) => {
      let Enddate = new Date().getTime();
      console.log(Enddate);
      this.attrGroups = [];
      this.dataForm.catelogId = path[path.length - 1];
    ...
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.attrId) {
          this.$http({
            url: this.$http.adornUrl(
              this.$url.attr + `info/${this.dataForm.attrId}`
            ),
            method: "get",
            params: this.$http.adornParams()
          }).then(({ data }) => {
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          this.$http({
            url: this.$http.adornUrl(
              this.$url.attr + `${!this.dataForm.attrId ? "save" : "update"}`
            ),
            method: "post",
  ...
  created() {
    ...
    console.log("添加修改子组件被创建");
  },
  activated() {
    console.log("添加修改子组件被缓存");
  }
};
</script>
