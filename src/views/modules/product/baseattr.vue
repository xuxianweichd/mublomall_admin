          ...
          >
            <template slot-scope="scope">
              <el-button
                type="text"
                size="small"
                @click="addOrUpdateHandle(scope.row.attrId,scope.row.attrType)"
              >修改</el-button>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.$http({
        // url: this.$http.adornUrl(this.$url.attr+`${type}/list/${this.catId}`),
        url: this.$http.adornUrl(this.$url.attr + `${type}/list/${this.catId}`),
        method: "get",
